        ...
        public int Solve()
        {
            ...
            foreach (string randomNumber in randomNumbers)
            {
                for (int i = boards.Count - 1; i >= 0; --i)
                {
                    ...
                    {
                        // If board is finished, remove it
                        // Last board is safely finished, so move it to current
                        // place and remove last.
                        boards[i] = boards[boards.Count - 1];
                        boards.RemoveAt(boards.Count - 1);

                        if (boards.Count == 0) // Last removed
