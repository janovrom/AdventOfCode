...
namespace AdventOfCode.Solutions.Day19
{
    internal class SolutionPart2 : IntSolution
    {
        ...
        private int RansacScanners()
        {
            ...
            Scanner[] storedScanners = _scanners.ToArray();
            ...
            foreach (Scanner s0 in storedScanners)
            {
                foreach (Scanner s1 in storedScanners)
                {
                    if (s0 == s1)
                        continue;

                    // Just to be sure that it's an integer and also to avoid any issues with adding floating points
                    int x = Math.Abs((int)Math.Round(s0.Center.X) - (int)Math.Round(s1.Center.X));
                    int y = Math.Abs((int)Math.Round(s0.Center.Y) - (int)Math.Round(s1.Center.Y));
                    int z = Math.Abs((int)Math.Round(s0.Center.Z) - (int)Math.Round(s1.Center.Z));

                    int d = x + y + z;

                    maxManhattanDistance = Math.Max(maxManhattanDistance, d);
                }
            }
